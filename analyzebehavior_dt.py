...
class AnalyzeAnimalDetector():
	...
	def categorize_behaviors(self,path_to_categorizer,uncertain=0,min_length=None):
		...
		for animal_name in self.animal_kinds:
			...
			idx=0
			for n in IDs:
				count=self.register_counts[animal_name].get(n)
				if count is None:
					count=0
				i=self.length+count
				idx+=i
		...
		if min_length is not None:
			for animal_name in self.animal_kinds:
				IDs=list(self.event_probability[animal_name].keys())

				for n in IDs:
					count=self.register_counts[animal_name].get(n)

					# Skip IDs that were never actually registered
					if count is None:
						continue
					...
					# Handle the final bout at the end
					if continued_length<min_length:
						self.event_probability[animal_name][n][i-continued_length:i]=[['NA',-1]]*continued_length
		...
		if len(parameter_to_analyze)>0:
			...
			for animal_name in self.animal_kinds:
				...
				for i in self.animal_contours[animal_name]:
					...
					count = self.register_counts[animal_name][i]
					if count is None:
						count = 0
					n = self.length + count
	...
	def _safe_sheet_name(self,name,max_length=31):

		safe=''.join(ch if ch not in ['\\','/','*','?','[',']',':'] else '_' for ch in str(name))
		return safe[:max_length] if len(safe)>max_length else safe


	def _parse_video_filename_metadata(self):
		...
		if len(parts)>=4:
			camera_id=parts[1]
			try:
				recorded_datetime=datetime.datetime.strptime(parts[3],'%Y%m%d%H%M%S')
			except ValueError:
				recorded_datetime=None
		return {'video_name':filename,'camera_id':camera_id,'recorded_datetime':recorded_datetime}


	def _animal_id_label(self,animal_name,animal_id):

		return str(animal_name)+'_'+str(animal_id)


	def _clone_event_probability(self,event_probability=None):

		if event_probability is None:
			event_probability=self.event_probability

		cloned={}
		for animal_name,id_dict in event_probability.items():
			cloned[animal_name]={}
			for animal_id,events in id_dict.items():
				cloned[animal_name][animal_id]=[]
				for event in events:
					if isinstance(event,(list,tuple)) and len(event)>=2:
						cloned[animal_name][animal_id].append([event[0],event[1]])
					else:
						cloned[animal_name][animal_id].append(['NA',None])
		return cloned


	def backfill_behavior_gaps(self,max_gap=None,event_probability=None):

		# Fill NA gaps only when the behavior before and after the gap is the same.
		# This preserves genuine behavior transitions while repairing brief dropouts.

		if not self.categorize_behavior:
			return self._clone_event_probability(event_probability if event_probability is not None else {})

		filled=self._clone_event_probability(event_probability)

		for animal_name in filled:
			for animal_id,events in filled[animal_name].items():
				idx=0
				while idx<len(events):
					...
					gap_start=idx
					while idx<len(events) and events[idx][0]=='NA':
						idx+=1
					gap_end=idx
					...
					right_idx=gap_end
					if left_idx<0 or right_idx>=len(events):
						continue

					left_behavior,left_conf=events[left_idx]
					right_behavior,right_conf=events[right_idx]

					if left_behavior=='NA' or right_behavior=='NA':
						continue
					...
					if left_conf is None and right_conf is None:
						fill_conf=None
					elif left_conf is None:
						fill_conf=right_conf
					elif right_conf is None:
						fill_conf=left_conf
					else:
						fill_conf=max(left_conf,right_conf)

					for fill_idx in range(gap_start,gap_end):
						events[fill_idx]=[left_behavior,fill_conf]
	...
	def _build_metadata_df(self):
		...
		file_metadata=self._parse_video_filename_metadata()
		duration_seconds=0 if len(self.all_time)==0 else max(self.all_time)
		rows=[
			{'variable':'video_name','value':file_metadata['video_name']},
			{'variable':'camera_id','value':file_metadata['camera_id']},
			{'variable':'recorded_datetime','value':file_metadata['recorded_datetime'].strftime('%Y-%m-%d %H:%M:%S') if file_metadata['recorded_datetime'] is not None else None},
			{'variable':'fps','value':self.fps},
			{'variable':'analyzed_frames','value':len(self.all_time)},
			{'variable':'duration_seconds','value':duration_seconds},
			{'variable':'analysis_start_time_seconds','value':self.t},
			{'variable':'analysis_requested_duration_seconds','value':self.duration},
			{'variable':'framewidth','value':self.framewidth},
			{'variable':'frameheight','value':self.frameheight},
			{'variable':'behavior_mode','value':self.behavior_mode},
			{'variable':'categorize_behavior','value':self.categorize_behavior},
			{'variable':'animation_analyzer','value':self.animation_analyzer},
		]
		for animal_name in self.animal_kinds:
			rows.append({'variable':str(animal_name)+'_n','value':self.animal_number[animal_name]})
		return pd.DataFrame(rows)


	def _build_tracking_df(self):

		rows=[]
		for animal_name in self.animal_kinds:
			for animal_id in self.animal_centers[animal_name]:
				id_label=self._animal_id_label(animal_name,animal_id)
				heights=self.animal_heights[animal_name][animal_id] if animal_id in self.animal_heights[animal_name] else [None]*len(self.all_time)
				for idx,time in enumerate(self.all_time):
					center=self.animal_centers[animal_name][animal_id][idx] if idx<len(self.animal_centers[animal_name][animal_id]) else None
					if center is None:
						x_val=None
						y_val=None
						detected=False
						...
						y_val=center[1]
						detected=True
					height_val=heights[idx] if idx<len(heights) else None
					rows.append({
						'time':time,
	...
	def _build_behavior_predictions_df(self):

		rows=[]
		if not self.categorize_behavior:
			return pd.DataFrame(columns=['time','animal_type','ID','predicted_behavior','confidence'])
		for animal_name in self.animal_kinds:
			for animal_id in self.event_probability[animal_name]:
				id_label=self._animal_id_label(animal_name,animal_id)
				for idx,time in enumerate(self.all_time):
					behavior='NA'
					confidence=None
					if idx<len(self.event_probability[animal_name][animal_id]):
						event=self.event_probability[animal_name][animal_id][idx]
						if isinstance(event,(list,tuple)) and len(event)>=2:
							behavior=event[0]
							confidence=event[1]
					rows.append({
	...
	def _build_behavior_probabilities_df(self):

		rows=[]
		if not self.categorize_behavior:
			return pd.DataFrame(columns=['time','animal_type','ID','behavior','probability'])
		for animal_name in self.animal_kinds:
			for behavior_name in self.all_behavior_parameters[animal_name]:
				probability_dict=self.all_behavior_parameters[animal_name][behavior_name]['probability']
				for animal_id in probability_dict:
					id_label=self._animal_id_label(animal_name,animal_id)
					for idx,time in enumerate(self.all_time):
						probability=None
						if idx<len(probability_dict[animal_id]):
							probability=probability_dict[animal_id][idx]
						rows.append({
							'time':time,
							'animal_type':animal_name,
							'ID':id_label,
	...
	def _build_movement_metrics_df(self,parameter_to_analyze):

		rows=[]
		if self.categorize_behavior:
			frame_parameters=[]
			if '3 length parameters' in parameter_to_analyze:
				frame_parameters+=['intensity_length','magnitude_length','vigor_length']
			if '3 areal parameters' in parameter_to_analyze:
				frame_parameters+=['intensity_area','magnitude_area','vigor_area']
			if '4 locomotion parameters' in parameter_to_analyze:
				frame_parameters+=['acceleration','speed','velocity']
			for animal_name in self.animal_kinds:
				for behavior_name in self.all_behavior_parameters[animal_name]:
					for parameter_name in frame_parameters:
						parameter_dict=self.all_behavior_parameters[animal_name][behavior_name].get(parameter_name,{})
						for animal_id in parameter_dict:
							id_label=self._animal_id_label(animal_name,animal_id)
							for idx,time in enumerate(self.all_time):
								value=parameter_dict[animal_id][idx] if idx<len(parameter_dict[animal_id]) else None
								rows.append({
									'time':time,
									'animal_type':animal_name,
									'ID':id_label,
			...
			for animal_name in self.animal_kinds:
				for parameter_name in frame_parameters:
					parameter_dict=self.all_behavior_parameters[animal_name].get(parameter_name,{})
					for animal_id in parameter_dict:
						id_label=self._animal_id_label(animal_name,animal_id)
						for idx,time in enumerate(self.all_time):
							value=parameter_dict[animal_id][idx] if idx<len(parameter_dict[animal_id]) else None
	...
	def _build_behavior_summary_df(self,parameter_to_analyze):

		rows=[]
		if self.categorize_behavior:
			for animal_name in self.animal_kinds:
				for behavior_name in self.all_behavior_parameters[animal_name]:
					params=self.all_behavior_parameters[animal_name][behavior_name]
					all_ids=set()
					for parameter_name,parameter_values in params.items():
						if isinstance(parameter_values,dict) and parameter_name!='color':
							all_ids.update(parameter_values.keys())
					for animal_id in sorted(all_ids):
						id_label=self._animal_id_label(animal_name,animal_id)
						row={'animal_type':animal_name,'ID':id_label,'behavior':behavior_name}
						if 'count' in parameter_to_analyze:
							row['count']=params['count'].get(animal_id)
						if 'duration' in parameter_to_analyze:
							row['duration_seconds']=params['duration'].get(animal_id)
						if 'latency' in parameter_to_analyze:
							row['latency_seconds']=params['latency'].get(animal_id)
						if '4 locomotion parameters' in parameter_to_analyze:
							row['distance']=params['distance'].get(animal_id)
						for metric in ['acceleration','speed','velocity','intensity_length','magnitude_length','vigor_length','intensity_area','magnitude_area','vigor_area','probability']:
							if metric in params and isinstance(params[metric],dict) and animal_id in params[metric]:
								values=pd.Series(params[metric][animal_id]).dropna()
								if len(values)>0:
			...
			for animal_name in self.animal_kinds:
				all_ids=set()
				for parameter_name,parameter_values in self.all_behavior_parameters[animal_name].items():
					if isinstance(parameter_values,dict):
						all_ids.update(parameter_values.keys())
				for animal_id in sorted(all_ids):
					id_label=self._animal_id_label(animal_name,animal_id)
					row={'animal_type':animal_name,'ID':id_label}
					if '4 locomotion parameters' in parameter_to_analyze:
						row['distance']=self.all_behavior_parameters[animal_name]['distance'].get(animal_id)
					for metric in ['acceleration','speed','velocity','intensity_length','magnitude_length','vigor_length','intensity_area','magnitude_area','vigor_area']:
						if metric in self.all_behavior_parameters[animal_name] and animal_id in self.all_behavior_parameters[animal_name][metric]:
							values=pd.Series(self.all_behavior_parameters[animal_name][metric][animal_id]).dropna()
							if len(values)>0:
								row[metric+'_mean']=values.mean()
								row[metric+'_max']=values.max()
								row[metric+'_min']=values.min()
	...
	def _seconds_to_next_hour_boundary(self,chunk_start_seconds):

		file_metadata=self._parse_video_filename_metadata()
		recorded_datetime=file_metadata.get('recorded_datetime')
		if recorded_datetime is None:
			return float('inf')

		chunk_start_dt=recorded_datetime+datetime.timedelta(seconds=float(chunk_start_seconds))
		next_hour=chunk_start_dt.replace(minute=0,second=0,microsecond=0)+datetime.timedelta(hours=1)
		return max(1.0,(next_hour-chunk_start_dt).total_seconds())


	def _filter_time_window(self,df,start_s,end_s):
		...
		if 'time' not in df.columns:
			return df.copy()
		return df[(df['time']>=start_s) & (df['time']<end_s)].copy()


	def _rebase_time_column(self,df,chunk_start_s):
		...
		if 'time' in out.columns:
			out.loc[:,'time']=out['time']-chunk_start_s
		return out


	def _build_chunk_metadata_df(self,chunk_start_s,chunk_duration_s,chunk_end_s,part_index):

		file_metadata=self._parse_video_filename_metadata()
		recorded_datetime=file_metadata.get('recorded_datetime')
		chunk_recorded_datetime=None
		if recorded_datetime is not None:
			chunk_recorded_datetime=recorded_datetime+datetime.timedelta(seconds=float(chunk_start_s))

		video_name=file_metadata.get('video_name')
		if video_name is None:
			video_name=self.basename
		video_stub,video_ext=os.path.splitext(video_name)
		if video_ext=='':
			video_ext='.mp4'
		chunk_video_name=f'{video_stub}_part{part_index:03d}{video_ext}'

		rows=[
			{'variable':'video_name','value':chunk_video_name},
			{'variable':'camera_id','value':file_metadata.get('camera_id')},
			{'variable':'recorded_datetime','value':chunk_recorded_datetime.strftime('%Y-%m-%d %H:%M:%S') if chunk_recorded_datetime is not None else None},
			{'variable':'fps','value':self.fps},
			{'variable':'analyzed_frames','value':int(round(max(0.0,chunk_end_s-chunk_start_s)*self.fps)) if self.fps is not None else None},
			{'variable':'duration_seconds','value':round(max(0.0,chunk_end_s-chunk_start_s),2)},
			{'variable':'analysis_start_time_seconds','value':round(chunk_start_s,2)},
			{'variable':'analysis_requested_duration_seconds','value':round(chunk_duration_s,2)},
			{'variable':'framewidth','value':self.framewidth},
			{'variable':'frameheight','value':self.frameheight},
			{'variable':'behavior_mode','value':self.behavior_mode},
			{'variable':'categorize_behavior','value':self.categorize_behavior},
	...
	def _build_behavior_summary_from_predictions(self,pred_df):

		columns=['animal_type','ID','behavior','count','duration_seconds','probability_mean','probability_max','probability_min']
		if pred_df is None or len(pred_df)==0:
			return pd.DataFrame(columns=columns)

		df=pred_df.copy()
		if 'predicted_behavior' not in df.columns:
			return pd.DataFrame(columns=columns)
		df=df[df['predicted_behavior'].notna()].copy()
		df=df[df['predicted_behavior']!='NA'].copy()
		if len(df)==0:
			return pd.DataFrame(columns=columns)

		frame_step=(1.0/self.fps) if self.fps not in [None,0] else np.nan
		df=df.sort_values(['animal_type','ID','time']).copy()
		df['prev_behavior']=df.groupby(['animal_type','ID'])['predicted_behavior'].shift(1)
		df['new_bout']=((df['prev_behavior'].isna()) | (df['predicted_behavior']!=df['prev_behavior'])).astype(int)

		summary_df=(
			df.groupby(['animal_type','ID','predicted_behavior'],dropna=False)
			.agg(
				count=('new_bout','sum'),
				duration_seconds=('predicted_behavior',lambda x: len(x)*frame_step),
				probability_mean=('confidence','mean'),
				probability_max=('confidence','max'),
				probability_min=('confidence','min')
			)
			.reset_index()
			.rename(columns={'predicted_behavior':'behavior'})
		)
		return summary_df


	def _write_analysis_workbook(self,workbook_path,metadata_df,tracking_df,behavior_predictions_df,behavior_probabilities_df,movement_metrics_df,behavior_summary_df,raw_behavior_predictions_df=None):
		...
		with pd.ExcelWriter(workbook_path) as writer:
			...
			if tracking_df is not None and len(tracking_df)>0:
				...
				pd.DataFrame(columns=['time','animal_type','ID','x','y','detected','height']).to_excel(writer,sheet_name='tracking',index=False)

			if self.categorize_behavior:
				if behavior_predictions_df is not None and len(behavior_predictions_df)>0:
					behavior_predictions_df.to_excel(writer,sheet_name='behavior_predictions',index=False)
				else:
					pd.DataFrame(columns=['time','animal_type','ID','predicted_behavior','confidence']).to_excel(writer,sheet_name='behavior_predictions',index=False)

				if behavior_probabilities_df is not None and len(behavior_probabilities_df)>0:
					behavior_probabilities_df.to_excel(writer,sheet_name='behavior_probabilities',index=False)
				else:
					pd.DataFrame(columns=['time','animal_type','ID','behavior','probability']).to_excel(writer,sheet_name='behavior_probabilities',index=False)

				if raw_behavior_predictions_df is not None:
					if len(raw_behavior_predictions_df)>0:
						raw_behavior_predictions_df.to_excel(writer,sheet_name='behavior_predictions_raw',index=False)
					else:
						pd.DataFrame(columns=['time','animal_type','ID','predicted_behavior','confidence']).to_excel(writer,sheet_name='behavior_predictions_raw',index=False)

			if movement_metrics_df is not None and len(movement_metrics_df)>0:
				movement_metrics_df.to_excel(writer,sheet_name='movement_metrics',index=False)
			else:
				pd.DataFrame(columns=['time','animal_type','ID','behavior','parameter','value']).to_excel(writer,sheet_name='movement_metrics',index=False)

			if behavior_summary_df is not None and len(behavior_summary_df)>0:
				behavior_summary_df.to_excel(writer,sheet_name='behavior_summary',index=False)
			else:
				pd.DataFrame(columns=['animal_type','ID','behavior','count','duration_seconds']).to_excel(writer,sheet_name='behavior_summary',index=False)


	def export_results(self,normalize_distance=True,parameter_to_analyze=[],backfill_behavior_gaps=True,max_backfill_gap=None,split_workbooks=False,max_chunk_seconds=600,align_to_hour=True):

		# normalize_distance: whether to normalize the distance (in pixel) to the animal contour area
		# parameter_to_analyze: the behavior parameters that are selected in the analysis
		# backfill_behavior_gaps: whether to fill NA gaps when the same behavior occurs on both sides
		# max_backfill_gap: maximum NA gap length (in frames) to fill; None fills all qualifying gaps
		# split_workbooks: whether to export multiple smaller xlsx files
		# max_chunk_seconds: maximum duration of each output workbook in seconds
		# align_to_hour: whether to stop each chunk at the next hour boundary according to recorded_datetime

		print('Preparing behavior predictions for export...')
		self.log.append('Preparing behavior predictions for export...')
		print(datetime.datetime.now())
		self.log.append(str(datetime.datetime.now()))

		raw_behavior_predictions_df=self._build_behavior_predictions_df()
		original_event_probability=None
		if self.categorize_behavior and backfill_behavior_gaps:
			original_event_probability=self._clone_event_probability()
			self.event_probability=self.backfill_behavior_gaps(max_gap=max_backfill_gap)
		...
		tracking_df=self._build_tracking_df()
		behavior_predictions_df=self._build_behavior_predictions_df()
		behavior_probabilities_df=self._build_behavior_probabilities_df()
		movement_metrics_df=self._build_movement_metrics_df(parameter_to_analyze)
		behavior_summary_df=self._build_behavior_summary_df(parameter_to_analyze)

		base_workbook_path=os.path.join(self.results_path,os.path.splitext(self.basename)[0]+'_analysis.xlsx')

		if split_workbooks is False:
			raw_df_to_write=raw_behavior_predictions_df if (self.categorize_behavior and backfill_behavior_gaps) else None
			self._write_analysis_workbook(
				base_workbook_path,
				metadata_df,
				tracking_df,
				behavior_predictions_df,
				behavior_probabilities_df,
				movement_metrics_df,
				behavior_summary_df,
				raw_behavior_predictions_df=raw_df_to_write
			)
			exported_paths=[base_workbook_path]
		else:
			if max_chunk_seconds is None or max_chunk_seconds<=0:
				max_chunk_seconds=1200

			exported_paths=[]
			if len(self.all_time)==0:
				total_duration=0.0
			else:
				total_duration=max(self.all_time)
			frame_step=(1.0/self.fps) if self.fps not in [None,0] else 0.0
			chunk_start=0.0
			part_index=1
			base_stub=os.path.splitext(self.basename)[0]

			while chunk_start<=total_duration:
				chunk_length=float(max_chunk_seconds)
				if align_to_hour:
					chunk_length=min(chunk_length,self._seconds_to_next_hour_boundary(chunk_start))
				chunk_end=min(total_duration+frame_step,chunk_start+chunk_length)
				if chunk_end<=chunk_start:
					break

				tracking_chunk=self._rebase_time_column(self._filter_time_window(tracking_df,chunk_start,chunk_end),chunk_start)
				predictions_chunk=self._rebase_time_column(self._filter_time_window(behavior_predictions_df,chunk_start,chunk_end),chunk_start)
				probabilities_chunk=self._rebase_time_column(self._filter_time_window(behavior_probabilities_df,chunk_start,chunk_end),chunk_start)
				movement_chunk=self._rebase_time_column(self._filter_time_window(movement_metrics_df,chunk_start,chunk_end),chunk_start)
				raw_predictions_chunk=None
				if self.categorize_behavior and backfill_behavior_gaps:
					raw_predictions_chunk=self._rebase_time_column(self._filter_time_window(raw_behavior_predictions_df,chunk_start,chunk_end),chunk_start)

				metadata_chunk=self._build_chunk_metadata_df(chunk_start,chunk_end-chunk_start,chunk_end,part_index)
				behavior_summary_chunk=self._build_behavior_summary_from_predictions(predictions_chunk)

				workbook_path=os.path.join(self.results_path,f'{base_stub}_analysis_part{part_index:03d}.xlsx')
				self._write_analysis_workbook(
					workbook_path,
					metadata_chunk,
					tracking_chunk,
					...
					probabilities_chunk,
					movement_chunk,
					behavior_summary_chunk,
					raw_behavior_predictions_df=raw_predictions_chunk
				)
		...
		if original_event_probability is not None:
			self.event_probability=original_event_probability

		for workbook_path in exported_paths:
			print('All results exported in: '+str(workbook_path))
			self.log.append('All results exported in: '+str(workbook_path))
